...
i <- read_csv("C:/Users/G50/Desktop/SRBCC/SRBCC/Datasets_Originales/ORIGINAL_INVENTARIO.csv", locale = readr::locale(encoding = "latin1"))
...
i$autorPrincipal <-tolower(i$autorPrincipal)
...
i$autores <- gsub("ô", "", i$autores)

 i$autores <-tolower(i$autores)
     i$materia <-tolower(i$materia)
# Se eliminan las 2 columnas
i$autorprincipal <- NULL
i$autorSecundario <- NULL


i$materia <- gsub("xa", "x", i$materia)
...
i
 ...
 i$materia <- gsub("[?]", "n", i$materia)
     ...
     i$autores <- gsub(" ", "", i$autores)
     i$autores <- gsub("  ", "", i$autores)
     i$autores <- gsub("na,", "", i$autores)
     i$autores <- gsub(",na", " ", i$autores)
     i$autores <- gsub(" ,", ", ", i$autores)
    i %>% View()
...
#i%>% View()
  ...
  #Cambia la seccion idioma a ENGLISH
    ...
    i %>% filter(Titulo == "limpieza de sangre")
    i %>% filter(Titulo == "economia y sociedad")
    
    
    chingar <- unique(i$Titulo)
    
    chingar %>% filter(Titulo == "limpieza de sangre")  
    
    
    chingar %>% View() 
 #write.csv(i, "Clean_Datasets/inventario_clean.csv")
    
  # i <- read_csv("C:/Users/G50/Desktop/RecommenderSystem/Clean_Datasets/inventario_clean.csv", locale = readr::locale(encoding = "latin1"))
    
   # spanish_i <- i %>% 
   # filter(language_reference == "spanish")
     #write.csv(spanish_i, "Inventario_English.spanish/inventario_spanish.csv")
    
   # english_i <- i %>%
    #filter(language_reference == "english")
    #write.csv(english_i, "Inventario_English.spanish/inventario_english.csv")
  #  
    #i %>% View()

    
    x <- read_csv("C:/Users/G50/Desktop/SRBCC/SRBCC/Inventario_English.spanish/inventario_spanishs.csv", locale = readr::locale(encoding = "latin1"))
    
    data %>% View()
    
    
    
    data <- subset(x, !duplicated(x[,"Titulo"]))
    
                  
    write.csv(data, "Inventario_English.spanish/inventario_spanish_noRepeatedData.csv")
    
    x$Titulo <- unique(x$Titulo)
